...
void treeprint_levelorder_save(tree root, int level, vector<int>& v) {
    DPRINT(cout << ">treeprint_levelorder lv=" << level << endl;);
    if (root == nullptr) return;
    if (level == 1)
        v.push_back(root->key);
    else {
        treeprint_levelorder_save(root->left, level - 1, v);
        treeprint_levelorder_save(root->right, level - 1, v);
    }
    DPRINT(cout << "<treeprint_levelorder lv=" << level << endl;);
}
...
void treeprint_levelorder_tasty(tree root) {
	...
	for (int i = 1; i <= 3; i++) {
		treeprint_levelorder_save(root, i, v);
        show_vector(v, 20);
        v.clear();
		cout << endl;
	}
