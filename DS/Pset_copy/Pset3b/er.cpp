...
int binarysearch(int *data, int key, int lo, int hi) {
	...
	//cout << "mid: " << mid << endl;
	//cout << "data[lo]: " << data[lo] << " data[hi]: " << data[hi] << " data[mid]: " << data[mid]  << endl;
	...
	if (key == data[mid])
		...
		//cout << "key is big" << endl;
	}
	else{
		return binarysearch(data, key, lo, mid);
		//cout << "mid is big" << endl;
	}
}
...
// extra function at user's convenience
void binarysearch(int *data, int key, int size){
	int idx = binarysearch(data, key, 0, size);
	if (idx < 0) 
		cout << "\t" << key << "\tis not @list[" << -idx << "]" << endl;
	else
		cout << "\t" << key << "\tis     @list[" << idx << "]" << endl;
