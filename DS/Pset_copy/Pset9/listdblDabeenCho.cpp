...
pNode half(pList p) {
    ...
    while (rabbit != last(p) && rabbit != end(p)) {
...
void erase(pList p, pNode x) {	// checks if x is either tail or head
	if (x == p->tail || x == p->head || x == nullptr)
        return;
	x->prev->next = x->next;
	x->next->prev = x->prev;
	delete x;
...
void push_backN(pList p, int N) {
    ...
    for (int i = 0; i < N; i++)
        push_back(p, rand_extended() % (range+1));
...
void unique(pList p) {
	...
	if (size(p) <= 1)
        return;
    Node *temp;
    for (Node *x = begin(p)->next; x != end(p);) {
...
bool sorted(pList p, bool (*comp)(int a, int b)) {
	...
	if (size(p) <= 1)
        return true;  // since ascending or descending
    int data = begin(p)->data;
    for (Node *x = begin(p)->next; x != end(p); x = x->next) {
        if (!comp(data, x->data))
            return false;
        data = x->data;
    }
...
void show(pList p, bool all, int show_n) {
    ...
    for (i = 1, curr = last(p); i < show_n ; i++)
            curr = curr->prev;
    while (curr != end(p)) {
            cout << "\t" << curr->data;
            curr = curr->next;
