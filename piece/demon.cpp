...
string file_content_to_report(const char * filename, std::string& report_str);



/* HAVE TO FIX: useless global variable */
bool register_fail = true;
...
void register_routine(string repo_owner, string repo_name){
		...
		/* Have to fix: fake command */
		title = "Register report";
		if (register_fail) {
			file_content_to_report("fake/register_fail", report);
			register_fail = false;
		}
		else {
			file_content_to_report("fake/register_pass", report);
		}
		/* Have to fix: fake command */
...
void finish_routine(string repo_owner, string repo_name){
		...
		/* Have to fix: fake command */
		title = "Finish report";
		file_content_to_report("fake/finish", report);
		/* Have to fix: fake command */
...
void submission_routine(int s_id, string repo_owner, string repo_name){
		...
		title = "Submit report";
		if (submit_fail) {
			file_content_to_report("fake/submit_fail", report);
			submit_fail = false;
		}
		else {
			file_content_to_report("fake/submit_pass", report);
		}
		/* Have to fix: fake command */
...
/* HAVE TO FIX: this is fake function */
...
string file_content_to_report(const char * filename, std::string& report_str) {
    ...
    while (getline(file, line)) {
        report_str += line; // Add each line to the report string
    }
